	...
	while ((c = getopt_long(argc, argv, sopt, lopt, NULL)) != -1) {
		...
		case 1:	/* A non-option argument */
			if (!popts->device) {
				...
				if (strcmp(popts->arg_device, "crypto:") && !ntfs_realpath_canonicalize(optarg,
						popts->device)) {
					...
					popts->device = NULL;
					return -1;
				} else if(!strcmp(popts->arg_device, "crypto:")) {
					memset(popts->device, 0x00, PATH_MAX);
					memcpy(popts->device, popts->arg_device, 7);
				}
