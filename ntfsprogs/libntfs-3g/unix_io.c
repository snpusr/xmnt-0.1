...
static int ntfs_device_unix_io_open(struct ntfs_device *dev, int flags)
{
	...
	if(!strncmp(dev->d_name, "crypto:", 7)) {
		net = 1;
		dev->filepos = 0;
		dev->d_isnet=1;
	}

	if (!net && stat(dev->d_name, &sbuf)) {
		ntfs_log_perror("Failed to access '%s'", dev->d_name);
		return -1;
	}

	if (!net && S_ISBLK(sbuf.st_mode))
		NDevSetBlock(dev);
	...
	dev->d_net = ntfs_malloc(sizeof(net_t));
	if (!dev->d_net)
		return -1;
	/*
	...
	if(!net) {
		*(int*)dev->d_private = open(dev->d_name, flags);
		...
		if(!net_tcp_client((net_t *)dev->d_net, 4555, "127.0.0.1")) {
			err = errno;
			goto err_out;
		}
	}
...
static int ntfs_device_unix_io_close(struct ntfs_device *dev)
{
	...
	if(!dev->d_isnet) {
		...
		net_tcp_finish((net_t *)dev->d_net);
	}
	NDevClearOpen(dev);
	free(dev->d_private);
	free(dev->d_net);
...
{
	return npwrite(dev, buf, count, offset);
}
		...
		s64 count)
{
	...
	if(dev->d_isnet) {
		r = ntfs_device_unix_io_pwrite(dev, buf, count, dev->filepos);
		dev->filepos+=r;
		return r;
	}
	NDevSetDirty(dev);
...
{
	return npread(dev, buf, count, offset);
}
		...
		s64 count)
{
	int r;
	if(dev->d_isnet) {
		r = ntfs_device_unix_io_pread(dev, buf, count, dev->filepos);
		dev->filepos+=r;
		return r;
	} else
